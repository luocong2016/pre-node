...
export interface ProAppsLogoComponentsToken extends ProAliasToken {
  componentCls: string
}

const genAppsLogoComponentsStyle: GenerateStyle<ProAppsLogoComponentsToken> = (token) => {
  return {
    [token.componentCls]: {
      "&-icon": {
        display: "inline-flex",
...
export default genComponentStyleHook("ProAppsLogoComponents", (token) => {
  return [genAppsLogoComponentsStyle(token)]
})
